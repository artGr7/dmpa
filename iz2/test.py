...
def get_angle_number(x, y):
    ...
    angle = int((np.arctan(y / x) + np.pi / 2) / (np.pi / 4)) % 8
    return angle
...
def canny(kernel_size, standard_deviation, bound_part, operator_mode, path):
    ...
    isCompleted = cv2.imwrite(".\\test_output\\" + file_name, img_border_filtered)
    if isCompleted:
...
def laplacian_of_gaussian(kernel_size, standard_deviation, bound, path):
    ...
    isCompleted = cv2.imwrite(".\\test_output\\" + file_name, thresh)
    if isCompleted:
        print("Complete " + file_name)
